...
def uneven_terrain_event():
    global hunger, thirst, health, objective, insanity, rations, guide, shotgun, time_until_cleared
    print('You come across uneven and steep terrain. Traversing it could hurt you but would be much faster.')
    response = get_user_input('What should you do next? (traverse the terrain/walk around): ', ['traverse the terrain', 'walk around'])
...
def handle_events():
    ...
    if insanity < 15:
        ...
        if event == 1: berry_bush_event()
        elif event == 2: uneven_terrain_event()
        elif event == 3: deer_event()
        elif event == 4: abandoned_cottage_event()
        elif event == 5: river_event()
        elif event == 6: fog_event()
        elif event == 7: creek_event()
        elif event == 8: bush_event()
        elif event == 9: old_man_event()
        elif event == 10: shotgun_event()
    else:
        wendigo_event()
